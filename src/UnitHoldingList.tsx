          ...
          <table className="w-full min-w-[1040px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-white text-left text-gray-700">
                ...
                <th className="px-2 py-3 font-semibold">Unit Holding ID</th>
            ...
            <tbody>
              {ROWS.map((row) => (
                <tr key={row.holdingId} className="border-b border-gray-100 text-gray-700 hover:bg-gray-50">
                  <td className="px-5 py-3">
                    <Checkbox size="sm" />
                  </td>
                  <td className="px-2 py-3 font-semibold text-gray-900">{row.holdingId}</td>
                  <td className="px-2 py-3">
                    <StatusPill status={row.status} />
                  </td>
                  <td className="px-2 py-3">{row.clientName}</td>
                  <td className="px-2 py-3">{row.sellerName}</td>
                  <td className="px-2 py-3">{row.project}</td>
                  <td className="px-2 py-3">{row.buildingUnit}</td>
                  <td className="px-2 py-3">{row.expiration}</td>
                  <td className="px-2 py-3">
                    <div className="flex items-center justify-end">
                      <button
                        type="button"
                        aria-label={`View ${row.holdingId} details`}
                        onClick={() => navigate({ screen: "unit-holding-details", itemId: row.holdingId })}
